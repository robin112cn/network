	...
	var handleValidation2 = function() {
		...
		return form2.validate({
					...
					highlight : function(element) { // hightlight error inputs
						$(element).closest('.form-group').removeClass("has-success").addClass('has-error'); // set error class to
						// the control group
					},
					...
					success : function(label, element) {
						var icon = Metronic.handValidStyle(element);
						$(element).closest('.form-group').removeClass('has-error').addClass('has-success'); // set success class
						// to the control
						// group
									...
									$.ajax({
												...
												success : function(data) {
													$.each(data, function(name, val) {
																$(		'#baseFieldsForm').find("[name='" + name + "']").val(val);
															});
													setFormStatus("edit");
