$(function() {
	$('#userPassWordAdd').keyup(function() {
		...
		var mediumRegex = new RegExp(
				"^(?=.{7,})(((?=.*[A-Z])(?=.*[a-z]))|((?=.*[A-Z])(?=.*[0-9]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[a-z])(?=.*\\W))|((?=.*[A-Z])(?=.*\\W))|((?=.*[0-9])(?=.*\\W))).*$",
				"g");
					...
					onDataLoad : function(grid) {
						...
						$("#grid tbody").find("tr").each(function() {
									...
									if ("admin" == userAcc) {
										...
										// $(this).find(".update").find("i").click(function
										// (event) {
										// return false;
										// });
									...
									$.ajax({
												...
												success : function(data) {
													if (data[0] == 0) {
														...
														$("#userIdAdd").val(data[1].userId);
														$("#userAccountAdd").val(data[1].userAccount);
														$("#userPassWordAdd").val(data[1].userPassword);
														$("#userPassWordRepeatAdd").val(data[1].userPassword);
														$("#userNameAdd").val(data[1].userName);
														$("#userSexAdd").val(data[1].userSex);
														$("#userRoleAdd").val(data[2]);
														$("#userEmailAdd").val(data[1].userEmail);
														$("#userPhoneAdd").val(data[1].userMobile);
														$("#userOFAdd").val(data[1].userOfficePhone);
														$("#userRemarkAdd").val(data[1].userRemark);
				...
				success : function(data) {
					...
					for (var i = 0; i < data.length; i++) {
						if (data[i].roleName != "系统管理员")
							$("#userRoleAdd").append("<option value='" + data[i].roleId + "'>" + data[i].roleName + "</option>");
					}
				}
			});
	$("#userIdAdd").val("");
	...
	var handleValidation2 = function() {
		...
		return form2.validate({
					...
					highlight : function(element) { // hightlight error inputs
						$(element).closest('.form-group').removeClass("has-success").addClass('has-error'); // set error class to the
						// control group
					},
